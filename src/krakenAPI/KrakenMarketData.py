# Get current server time.
# Endpoint does not require authentication,
# but has utility functions for authentication.

import http.client
import urllib.request
import urllib.parse
import hashlib
import hmac
import base64
import json
import time
KRAKEN_REST="https://api.kraken.com"
def request(method: str = "GET", path: str = "", query: dict | None = None, body: dict | None = None, public_key: str = "", private_key: str = "", environment: str = "") -> http.client.HTTPResponse:
   url = environment + path
   query_str = ""
   if query is not None and len(query) > 0:
      query_str = urllib.parse.urlencode(query)
      url += "?" + query_str
   nonce = ""
   if len(public_key) > 0:
      if body is None:
         body = {}
      nonce = body.get("nonce")
      if nonce is None:
         nonce = get_nonce()
         body["nonce"] = nonce
   headers = {}
   body_str = ""
   if body is not None and len(body) > 0:
      body_str = json.dumps(body)
      headers["Content-Type"] = "application/json"
   if len(public_key) > 0:
      headers["API-Key"] = public_key
      headers["API-Sign"] = get_signature(private_key, query_str+body_str, nonce, path)
   req = urllib.request.Request(
      method=method,
      url=url,
      data=body_str.encode(),
      headers=headers,
   )
   return urllib.request.urlopen(req)

def get_nonce() -> str:
   return str(int(time.time() * 1000))

def get_signature(private_key: str, data: str, nonce: str, path: str) -> str:
   return sign(
      private_key=private_key,
      message=path.encode() + hashlib.sha256(
            (nonce + data)
         .encode()
      ).digest()
   )

def sign(private_key: str, message: bytes) -> str:
   return base64.b64encode(
      hmac.new(
         key=base64.b64decode(private_key),
         msg=message,
         digestmod=hashlib.sha512,
      ).digest()
   ).decode()





####################### Server Time ###############################
# Get current server Time.
def GetServerTime():
   response = request(
      method="GET",
      path="/0/public/Time",
      environment=KRAKEN_REST,
   )
   return response

####################### System Status ###############################
# Get current system status.
def GetSystemStatus():
   response = request(
      method="GET",
      path="/0/public/SystemStatus",
      environment=KRAKEN_REST,
   )
   return response


##################################################################
#Query options: NOTE "" is invalid
#combinations can create invalid requests eg Pair must be within asset_class if both are specified 

####################### Asset Info ###############################
# Get current Asset Info.
def GetAssetInfo(query=None):
   if query is None:
      query = {}

   response = request(
      method="GET",
      path="/0/public/Assets",
      environment=KRAKEN_REST,
      query=query
   )
   return response


####################### Tradable Asset Pairs ###############################
# Get current Tradable Asset Pairs.
def GetTradableAssetPairs(query=None):
   if query is None:
      query = {}

   response = request(
      method="GET",
      path="/0/public/AssetPairs",
      environment=KRAKEN_REST,
      query=query
   )
   return response


####################### Ticker Information ###############################
# Get current Ticker Information.
def GetTickerInformation(query=None):
   if query is None:
      query = {}

   response = request(
      method="GET",
      path="/0/public/Ticker",
      environment=KRAKEN_REST,
      query=query#"tokenized_asset" or "forex"
   )
   return response

####################### OHLC Data ###############################
# Get current OHLC Data.
def GetOHLCData(query=None):
   if query is None:
      query = {}

   response = request(
      method="GET",
      path="/0/public/OHLC",
      environment=KRAKEN_REST,
      query=query#"tokenized_asset" or "forex"
   )
   return response

####################### Order Book ###############################
# Get current Order Book.
def GetOrderBook(query=None):
   if query is None:
      query = {}

   response = request(
      method="GET",
      path="/0/public/Depth",
      environment=KRAKEN_REST,
      query=query#"tokenized_asset" or "forex"
   )
   return response


####################### Recent Trades ###############################
# Get current Recent Trades.
def GetRecentTrades(query=None):
   if query is None:
      query = {}

   response = request(
      method="GET",
      path="/0/public/Trades",
      environment=KRAKEN_REST,
      query=query#"tokenized_asset" or "forex"
   )
   return response

####################### Recent Spreads ###############################
# Get current Recent Spreads.
def GetRecentSpreads(query=None):
   if query is None:
      query = {}

   response = request(
      method="GET",
      path="/0/public/Spread",
      environment=KRAKEN_REST,
      query=query#"tokenized_asset" or "forex"
   )
   return response