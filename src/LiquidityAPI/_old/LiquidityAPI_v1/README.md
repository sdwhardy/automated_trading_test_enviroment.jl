...
1. **Data Collection**: Fetches data from FRED, ECB SDMX, BIS, and IMF APIs
2. **Geographic Coverage**: US, Eurozone, China, Japan + global aggregates
...
- **M2 Money Stock** (`M2SL`) - Broad money supply
- **Federal Reserve Assets** (`WALCL`) - Fed balance sheet total
- **Securities Held Outright** (`WSHOSHO`) - QE program size
- **Bank Credit** (`TOTBKCR`) - Commercial bank lending
- **Consumer Credit** (`TOTALSL`) - Household credit
- **Treasury General Account** (`WTREGEN`) - Government cash balance (reverse liquidity)
- **Overnight Reverse Repo** (`RRPONTSYD`) - Fed liquidity drain
- **Currency in Circulation** (`CURRSL`) - Physical cash

### Eurozone (via FRED/ECB)
- **M2 Euro Area** (`MYAGM2EZM196N`) - Eurozone broad money
- **ECB Assets** (`ECBASSETSW`) - ECB balance sheet

### China (via FRED)
- **M2 China** (`MYAGM2CNM189N`) - Chinese money supply
- **PBoC Assets** (`DDDI06CNA156NWDB`) - People's Bank of China balance sheet

### Japan (via FRED)
- **M2 Japan** (`MYAGM2JPM189N`) - Japanese money supply
- **BoJ Assets** (`JPNASSETS`) - Bank of Japan balance sheet

### Global Indicators (via FRED/BIS)
- **USD 3M LIBOR** (`USD3MTD156N`) - Dollar funding costs
- **Fed Liquidity Swaps** (`SWPT`) - International USD provision
- **High Yield Spread** (`BAMLH0A0HYM2`) - Credit stress indicator
- **Investment Grade Spread** (`BAMLC0A0CM`) - Corporate credit conditions
- **TED Spread** (`TEDRATE`) - Interbank stress
- **VIX** (`VIXCLS`) - Risk appetite / liquidity demand
...
factor_df = pd.read_csv('global_liquidity_factor.csv', index_col=0, parse_dates=True)
returns_df = pd.read_csv('your_asset_returns.csv', index_col=0, parse_dates=True)
