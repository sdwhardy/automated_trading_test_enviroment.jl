...
def backtest_factor_strategy():
    ...
    # Load the factor
    factor = pd.read_csv('global_liquidity_factor.csv', 
...
def analyze_factor_asset_correlations():
    ...
    plt.tight_layout()
    plt.savefig('asset_correlations.png', dpi=300, bbox_inches='tight')
...
def identify_liquidity_regimes():
    ...
    plt.tight_layout()
    plt.savefig('liquidity_regimes.png', dpi=300, bbox_inches='tight')
...
def main():
    ...
    try:
        pd.read_csv('global_liquidity_factor.csv', nrows=1)
